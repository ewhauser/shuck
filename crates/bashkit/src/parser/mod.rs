...
impl<'a> Parser<'a> {
    ...
    fn word_from_token(&self, token: &tokens::Token, span: Span) -> Option<Word> {
        match token {
            tokens::Token::Word(w) => {
            ...
            tokens::Token::LiteralWord(w) => Some(Word::quoted_literal_with_span(w.clone(), span)),
            _ => None,
        }
    }
    ...
    fn is_recovery_separator(token: &tokens::Token) -> bool {
        matches!(
            token,
