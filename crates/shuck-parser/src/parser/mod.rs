...
impl<'a> Parser<'a> {
    ...
    fn word_with_parts(&self, parts: Vec<WordPartNode>, span: Span) -> Word {
        let surface_syntax = self.word_surface_syntax_from_parts(&parts, span);
    ...
    fn rebase_word(word: &mut Word, base: Position) {
        ...
        for brace in &mut word.surface_syntax.braces {
            brace.span = brace.span.rebased(base);
        }
        for template in &mut word.surface_syntax.escaped_parameter_templates {
            template.span = template.span.rebased(base);
            template.body_span = template.body_span.rebased(base);
        }
        for expansion in &mut word.surface_syntax.all_elements_array_expansions {
            expansion.span = expansion.span.rebased(base);
        }
        if let Some(zsh_surface_syntax) = &mut word.zsh_surface_syntax {
            for entry in &mut zsh_surface_syntax.short_positional_at {
                entry.span = entry.span.rebased(base);
                entry.base_span = entry.base_span.rebased(base);
                entry.suffix_span = entry.suffix_span.rebased(base);
            }
        }
