...
impl From<ParserParseDiagnostic> for ParseDiagnostic {
    fn from(diagnostic: ParserParseDiagnostic) -> Self {
        Self {
...
impl ParseError {
    fn from_parser_error(error: ParserError) -> Self {
        let ParserError::Parse {
            message,
            line,
            column,
        } = error;
...
/// Collect comments while preserving the parser's definition of when `#` starts a comment.
    ...
    let (script, diagnostics) = match (view.grammar, view.is_recovered()) {
        (Grammar::Bash, false) => {
            let script = Parser::new(input)
                .parse()
                .map_err(ParseError::from_parser_error)?;
            (script, Vec::new())
        }
        (Grammar::Bash, true) => {
