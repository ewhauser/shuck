...
pub(super) fn xargs_sc2267_default_replace_silent_shape(args: &[&Word], source: &str) -> bool {
    xargs_command_is_shell_c_wrapper(args, source)
        || xargs_command_is_echo_leading_dash_replacement(args, source)
}

pub(super) fn xargs_command_is_shell_c_wrapper(args: &[&Word], source: &str) -> bool {
    let args = if args
        .first()
        .and_then(|word| static_word_text(word, source))
        .as_deref()
...
pub(super) fn xargs_command_is_echo_leading_dash_replacement(args: &[&Word], source: &str) -> bool {
    let Some(command_name) = args.first().and_then(|word| static_word_text(word, source)) else {
        return false;
    };
...
pub(super) fn command_basename(name: &str) -> &str {
    name.rsplit('/').next().unwrap_or(name)
}

pub(super) fn xargs_long_option_argument(
    option: &str,
...
pub(super) fn long_name(option: &str) -> &str {
    option.split_once('=').map_or(option, |(name, _)| name)
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum XargsShortOptionArgumentStyle {
    None,
    OptionalInlineOnly,
    Required,
}

pub(super) fn xargs_short_option_argument_style(flag: char) -> XargsShortOptionArgumentStyle {
    match flag {
...
pub(super) fn xargs_long_option_requires_separate_argument(option: &str) -> bool {
    if option.contains('=') {
        return false;
    }
