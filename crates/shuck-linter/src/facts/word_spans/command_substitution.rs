...
pub(super) fn collect_command_substitution_spans(parts: &[WordPartNode], spans: &mut Vec<Span>) {
    for part in parts {
        match &part.kind {
...
pub(super) fn collect_unquoted_command_substitution_spans(
    parts: &[WordPartNode],
    quoted: bool,
    spans: &mut Vec<Span>,
) {
...
pub(super) fn collect_unquoted_dollar_paren_command_substitution_spans(
    parts: &[WordPartNode],
    quoted: bool,
    spans: &mut Vec<Span>,
) {
...
pub(super) fn normalize_command_substitution_span(span: Span, source: &str) -> Span {
...
pub(super) fn normalize_command_substitution_spans(spans: &mut [Span], source: &str) {
    for span in spans {
        *span = normalize_command_substitution_span(*span, source);
    }
}

pub(super) fn widen_dollar_paren_command_substitution_span(
    span: Span,
    source: &str,
) -> Option<Span> {
