...
pub fn leading_static_word_prefix_fix_in_source(
    word: &Word,
    source: &str,
    prefix_len: usize,
) -> Option<Fix> {
...
fn collect_leading_static_word_prefix_edits_from_part(
    ...
    let semantic = static_word_part_text(part, source)?;
...
fn static_word_part_text(part: &WordPartNode, source: &str) -> Option<String> {
    match &part.kind {
        ...
        WordPart::DoubleQuoted { parts, .. } => {
            ...
            for nested in parts {
                text.push_str(&static_word_part_text(nested, source)?);
            }
            Some(text)
        }
        _ => None,
