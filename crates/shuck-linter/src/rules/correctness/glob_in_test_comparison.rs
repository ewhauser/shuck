...
fn report_span(simple_test: &crate::SimpleTestFact<'_>, source: &str) -> Option<shuck_ast::Span> {
    if simple_test.syntax() != SimpleTestSyntax::Bracket
        || simple_test.shape() != SimpleTestShape::Binary
    {
        return None;
    }

    let operator = static_word_text(simple_test.operands().get(1)?, source)?;
    if !matches!(operator.as_str(), "=" | "==" | "!=") {
        return None;
    }

    let rhs = *simple_test.operands().get(2)?;
    let rhs_class = simple_test.operand_class(2)?;
