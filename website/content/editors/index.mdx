...
| Go to definition | `textDocument/definition` | File-scope variables and function calls resolve through literal sources, valid `# shuck: source=` hints, and configured source paths, with open buffers taking precedence. Function-local and transient variable shadows remain document-local, and conditional, dynamic, or unresolved sources are not guessed for variables. |
...
| Find references | `textDocument/references` | File-scope variable families and exact function bindings are followed across the statically resolved source graph, including source hints, configured source paths, cycles, and open buffers. Function-local and transient variable shadows stay local, conditional, dynamic, or unresolved sources are not guessed for variables, and `includeDeclaration` is honored. |
