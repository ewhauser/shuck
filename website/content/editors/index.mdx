...
| Completion | `textDocument/completion`, `completionItem/resolve` | Variables, declaration names, runtime names, builtins, shell keywords, and functions visible in the active document or through unconditional proven `source`/`.` edges. Literal paths, valid source hints, configured source paths, recognized current-file anchors, and unsaved open buffers participate; other dynamic or unresolved sources do not. Command-specific option completion is not supported. |
| Hover | `textDocument/hover` | Semantic symbol information plus help for suppression codes in `# shuck:` and `# shellcheck` directives. Function calls can resolve through the same exact source graph as go-to-definition, including source hints, configured source paths, recognized current-file anchors, and open buffers. Other dynamic or ambiguous sources are not guessed, and Shuck does not fetch command manuals. |
| Go to definition | `textDocument/definition` | File-scope variables and function calls resolve through literal sources, valid `# shuck: source=` hints, configured source paths, and recognized current-file anchors, with open buffers taking precedence. Function-local and transient variable shadows remain document-local, and conditional, other dynamic, or unresolved sources are not guessed for variables. |
| Document links | `textDocument/documentLink` | Resolvable literal `source`/`.` operands, valid source hints, and recognized current-file anchors link to their target files. Configured source paths and unsaved open buffers participate; other dynamic, missing, and out-of-workspace targets are omitted. |
| Find references | `textDocument/references` | File-scope variable families and exact function bindings are followed across the statically resolved source graph, including source hints, configured source paths, recognized current-file anchors, cycles, and open buffers. Function-local and transient variable shadows stay local, conditional, other dynamic, or unresolved sources are not guessed for variables, and `includeDeclaration` is honored. |
