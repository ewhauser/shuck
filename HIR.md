...
- Default to a normalized HIR, not a CST and not a thin wrapper over `bashkit` AST, because shell rules need stronger domain types than `bashkit` currently exposes.
...
- Keep parser-only details out of the rule API. Raw `bashkit` AST stays internal to lowering.
...
- Add a thin rule-facing facade in `shuck` so future rules depend on `HirFile` + `SemanticModel` + `ProjectClosure`, not `bashkit` AST
...
- Parity tests against the Go frontend on representative fixtures for:
  ...
  - project closure
- Acceptance criteria:
  - first rules can be written without touching raw `bashkit` AST
