...
Shuck is a shell script linter/checker CLI tool, built on top of **bashkit** (an in-process virtual bash interpreter written in Rust). The repo is a Cargo workspace containing both shuck (the linter) and bashkit (the underlying library).
...
# Build/test everything (including bashkit)
cargo build
...
cargo test -p bashkit -- test_name
...
- **`crates/bashkit`** — The upstream virtual bash interpreter library. Provides `Lexer`, `Parser`, AST types, and the full execution runtime. Shuck only uses the parser/lexer portion.
...
### bashkit parser internals

The parser (`crates/bashkit/src/parser/`) is a recursive descent parser with these modules:
